...
describe('performers', () => {
  it('provides T1 at a valid 8-to-5 marching-step position', () => {
    expect(performers).toEqual([
      expect.objectContaining({ id: 't1', label: 'T1', name: '', section: '' }),
    ])
  ...
  it('assigns sequential labels and unique IDs to new performers', () => {
    ...
    expect(p1).toMatchObject({ id: 'p1', label: 'P1', name: '', section: '' })
    expect(p2).toMatchObject({ id: 'p2', label: 'P2', name: '', section: '' })
    expect(performerMarkerRadiusSvg).toBe(6)
  })
  ...
  it('moves a selected group together without crossing field boundaries', () => {
    ...
    expect(movedPerformers.every(({ x, y }) =>
      x >= performerMarkerRadiusSvg
      && x <= fieldGeometry.svgWidth - performerMarkerRadiusSvg
      && y >= performerMarkerRadiusSvg
      && y <= fieldGeometry.svgHeight - performerMarkerRadiusSvg,
    )).toBe(true)
  })
  ...
  it('clamps a group with one shared snapped delta at the field boundary', () => {
    ...
    expect(movedPerformers[1].x).toBe(1193.75)
